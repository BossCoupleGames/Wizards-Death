...
namespace _Scripts
{
    ...
    {   [SerializeField] private  float timeDelay = 1f;
        ...
        [SerializeField] private AudioClip slimeHurtsound;
        ...
        public void TakeDamage(float damage)
        {
            ...
            {
                 _state = State.Hurt;
                 AudioSystem.Instance.PlaySound(slimeHurtsound,HurtVol);
                            _animator.SetTrigger("Hurt");
                            currentHealth -= damage;
                            if (currentHealth <= 0)
                            {
                                AudioSystem.Instance.PlaySound(slimeDeathsound,DeathVol);
                                Death();
                            }
            }
